...
export default function ShopCategoryPage() {
  ...
  React.useEffect(() => {
    async function loadData() {
      ...
      setError(null);
      try {
        const cat = await apiGet<Category>(`/api/categories/${categoryId}`);
        const normalized = { ...cat, show_by_brand: cat.show_by_brand !== false };
        setCategory(normalized);

        if (normalized.show_by_brand) {
          const brandList = await apiGet<Brand[]>("/api/brands");
          setBrands(brandList || []);
        } else {
          const productsRes = await apiGet<BrandGroup[]>(`/api/categories/${categoryId}/products-by-model`);
          setBrandGroups(productsRes || []);
        }
      } catch (err: any) {
        ...
        {loading ? (
          ...
          <div className="flex min-h-[60vh] items-center justify-center">
            <div className="text-center max-w-md mx-auto px-4">
              ...
              <button onClick={() => router.push("/shop")} className="px-6 py-3 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-xl font-medium hover:bg-gray-800 dark:hover:bg-gray-100 transition-all">
                Back to Shop
              </button>
            </div>
          </div>
            ...
            {category?.show_by_brand ? (
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {filteredBrands.map((brand, index) => (
                  <button
                    key={brand.id}
                    onClick={() => router.push(`/shop/brand/${brand.id}?category=${categoryId}`)}
