...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withErrorHandling(async () => {
    await requireRole(req, ["admin", "manager"]);
    const { id } = await params;
    const doc = await collections.brands().findOne({ _id: toObjectId(id) as any });
    if (!doc) throw ApiError.notFound("Brand not found");
    ...
    deleteCloudinaryAssets([doc?.logo_url]).catch((err) => console.error("[cloudinary] brand cleanup failed", err));
